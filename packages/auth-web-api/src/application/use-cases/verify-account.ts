import { IUserRepository, IVerificationTokenRepository } from "../ports";
import { UserNotFoundError, InvalidValidationTokenError } from "@common/errors";
...
class UseCase implements IVerifyAccountUseCase {
  ...
  async execute({ token: verificationToken }) {
    const token = await this.verificationTokenRepository.getTokenByTokenValue(
      verificationToken
    );
