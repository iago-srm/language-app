...
describe('Unit Tests for ActivityInstruction Entity', () => {
  it('Should not throw an error if valid parameters are passed to constructor.', () => {
    ...
    const sut2 = new ActivityInstruction({
      text: instructionMax,
      isCheckbox: false,
    });
  ...
  it('Should throw an error if invalid parameters are passed to constructor.', () => {
    try {
      new ActivityInstruction({
        text: 'a'.repeat(DomainRules.ACTIVITY.INSTRUCTION.MIN_LENGTH - 1),
        isCheckbox: true,
      });
    } catch (e) {
    ...
    try {
      new ActivityInstruction({
        text: 'a'.repeat(DomainRules.ACTIVITY.INSTRUCTION.MAX_LENGTH + 1),
        isCheckbox: true,
      });
    } catch (e) {
      expect(e).toMatchObject({
