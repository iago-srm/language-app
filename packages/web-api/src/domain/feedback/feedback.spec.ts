...
describe("Unit Tests for FeedbackToActivity Entity", () => {
  ...
  it("Should throw an error if too long of a message is passed", () => {
    try {
      new Feedback({ grade: DomainRules.FEEDBACK.GRADES[0], message: 'k'.repeat(DomainRules.FEEDBACK.MAX_LENGTH + 1)})
    } catch(e) {
  ...
  it("Should not throw errors and set values if valid values are passed", () => {
    const feedback = new Feedback({ grade: DomainRules.FEEDBACK.GRADES[0], message: 'ok' });
