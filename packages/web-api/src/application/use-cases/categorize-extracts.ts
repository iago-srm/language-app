...
export type ICategorizeExtractsUseCase = IUseCase<
...
export const CategorizeExtractsUseCaseFactory: ICategorizeExtractsUseCaseFactory =
  ({ extractsRepository, categoriesRepository }) => {
    return {
      execute: async ({ userId, category: categoryName, extractId }) => {
        const categoryDTO = (
          await categoriesRepository.getByNameFromUser(userId, categoryName)
        )[0];
        ...
        const categoryExtractDTOs = await extractsRepository.getByCategoryId(
          categoryDTO.id!
        );
        ...
        if (
          newExtractDTO.categoryId &&
          newExtractDTO.categoryId !== categoryDTO.id
        ) {
          const oldCategoryDTO = await categoriesRepository.getById(
            newExtractDTO.categoryId
          );
          const oldCategoryExtractDTOs =
            await extractsRepository.getByCategoryId(oldCategoryDTO!.id!);

          const oldCategory = new Category({
            total: oldCategoryDTO!.total,
            extracts: oldCategoryExtractDTOs.map(
              (dto) => new Extract({ id: dto.id })
            ),
          });

          oldCategory.removeExtract(
            new Extract({
              id: newExtractDTO.id,
          ...
          await categoriesRepository.updateCategory({
            ...
            extracts: oldCategoryExtractDTOs.filter(
              (e) => e.id !== newExtractDTO.id
            ),
          });
        }
        ...
        const category = new Category({
          ...
          extracts: categoryExtractDTOs.map(
            (dto) => new Extract({ id: dto.id })
          ),
        });

        category.addNewExtract(
          new Extract({
            id: newExtractDTO.id,
            amount: newExtractDTO.amount,
          })
