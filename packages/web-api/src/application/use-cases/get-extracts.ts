...
export const GetExtractsUseCaseFactory: IGetExtractsUseCaseFactory = ({
  ...
  return {
    execute: async ({ userId }) => {
      ...
      const newExtractDTOs = (
        await openBankingService.getExtracts(
          userDTO!.cpf,
          userDTO!.lastExtractFetch
        )
      ).map((extract) => ({ ...extract, user: userDTO }));
      ...
      return {
        extracts: extracts.map((extract) => ({ ...extract, user: undefined })),
      };
    },
  };
};
