...
class UseCase implements INewAssociationInvitationUseCase {
  ...
  async execute({ userId, studentEmail, language }) {
    const student = await this.studentRepository.getStudentByUserEmail(
      studentEmail
    );
    ...
    const existingInvitation =
      await this.associationInvitationTokenRepository.getTokenByStudentAndInstructorIds(
        instructor.id,
        student.id
      );
    ...
    const otherInvitationToThisStudent =
      await this.associationInvitationTokenRepository.getTokenByStudentId(
        student.id
      );
