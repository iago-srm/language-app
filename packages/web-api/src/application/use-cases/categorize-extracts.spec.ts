...
describe.skip('Categorizing an extract which is', () => {
  ...
  test('Uncategorized into an empty category', async () => {
    ...
    const categoryDTO = (
      await categoriesRepository.getAllFromUser(user.id)
    ).find((category) => category.name === testCategory.name);
    const extractDTO = await extractsRepository.getByCategoryId(
      categoryDTO!.id!
    );
  ...
  test.todo(
    'Categorized into the same category, category has this extract plus others'
  );
