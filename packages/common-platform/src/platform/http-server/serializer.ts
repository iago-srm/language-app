...
export const controllerSerializer = (
  body: { [k: string]: string },
  fields: (
    | { name: string; optional?: boolean; type?: "string" | "array" }
    | string
  )[]
) => {
  const values = {};
  ...
  fields.forEach((field) => {
    ...
    if (typeof field !== "string" && field.optional) {
      ...
      // string field
      else values[field.name] = body[field.name];
      return;
    }
    ...
    //non-optional field
    if (
      (typeof field !== "string" && !field.optional) ||
      typeof field === "string"
    ) {
      // field does not exist
      if (body[fieldName] === undefined)
        throw new ParameterNotProvidedError({ parameter: fieldName });
      // field is an array
      if (typeof field !== "string" && field.type === "array") {
